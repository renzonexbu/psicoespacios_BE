import { Controller, Post, Body, BadRequestException, Get, Param, Logger, Res, Query } from '@nestjs/common';
...
export class FlowController {
  ...
  async confirmarPago(@Body() body: any) {
    ...
    try {
      ...
      if (body.status !== 1) {
        this.logger.log(`Pago ${body.flowOrder} no exitoso (status: ${body.status}), no se procesará`);
      ...
      if (reservaTemporal) {
        this.logger.log(`Reserva temporal encontrada para flowOrder: ${body.flowOrder}, confirmando pago de sesión`);
        ...
        const resultado = await this.pagoSesionService.confirmarPagoFlow(body.flowOrder, datosPago);
        
        this.logger.log(`Pago de sesión confirmado: reserva ${resultado.reserva.id}, pago ${resultado.pago.id}`);
      ...
      this.logger.log(`Actualizando pago general ${pago.id} con status: ${body.status}`);
   ...
   */
  @Post('return')
  async retornoFlow(@Body() body: any, @Res() res: Response, @Query() query: any) {
    this.logger.log(`Retorno de Flow recibido: ${JSON.stringify({ body, query })}`);
    ...
    try {
      ...
      if (error === 'processing') {
        ...
        if (flowOrder) {
          reservaTemporal = await this.reservaRepository
            ...
            .andWhere("reserva.metadatos->>'flowOrderId' = :flowOrder", { flowOrder })
            .getOne();
        }
        ...
        if (reservaTemporal && reservaTemporal.estado !== EstadoReservaPsicologo.CANCELADA) {
          this.logger.log(`Cancelando reserva ${reservaTemporal.id} debido a error de procesamiento`);
          reservaTemporal.estado = EstadoReservaPsicologo.CANCELADA;
          await this.reservaRepository.save(reservaTemporal);
          this.logger.log(`Reserva ${reservaTemporal.id} cancelada exitosamente`);
          ...
          this.logger.log(`Redirigiendo a frontend con reserva cancelada: ${redirectUrl}`);
          ...
          this.logger.warn('No se encontró reserva para cancelar en caso de error de procesamiento');
          const frontUrl = this.getFrontUrl();
          return res.redirect(`${frontUrl}/agenda/confirmacion-sesion?error=processing`);
        }
      }
      ...
      this.logger.log(`Datos recibidos de Flow - flowOrder: ${flowOrder}, token: ${token}`);
      
      if (!flowOrder && !token) {
        this.logger.error('No se recibió flowOrder ni token en el retorno de Flow');
        // Redirigir al frontend de todas formas
        return res.redirect(`${this.getFrontUrl()}/agenda/confirmacion-sesion?error=no-order`);
      }
      ...
      try {
        ...
        if (token) {
          this.logger.log(`Consultando estado del pago en Flow con token: ${token}`);
          ...
          this.logger.log(`Estado del pago obtenido de Flow - status: ${statusReal}, flowOrder: ${estadoPago.flowOrder}, pagoExitoso: ${pagoExitoso}`);
        } else if (flowOrder) {
          this.logger.log(`Consultando estado del pago en Flow con flowOrder: ${flowOrder}`);
          const estadoPago = await this.flowService.getPaymentStatusByFlowOrder(flowOrder);
          statusReal = estadoPago.status;
          pagoExitoso = estadoPago.status === 2; // Según doc: 2 = pagada, 1 = pendiente
          
          this.logger.log(`Estado del pago obtenido de Flow - status: ${statusReal}, flowOrder: ${estadoPago.flowOrder}, pagoExitoso: ${pagoExitoso}`);
        } else {
          this.logger.warn('No hay token ni flowOrder para consultar estado del pago');
          pagoExitoso = false;
        }
      } catch (error) {
        this.logger.error(`Error al consultar estado del pago en Flow: ${error.message}`);
      ...
      this.logger.log(`Buscando reserva para flowOrder: ${flowOrder || token}, encontrada: ${reservaTemporal?.id || 'no encontrada'}`);
      ...
      if (!reservaTemporal) {
        ...
        this.logger.warn(`Reserva no encontrada para flowOrder: ${flowOrder || token}`);
        return res.redirect(redirectUrl);
      }

      // Verificar si el pago fue exitoso (status = 1) y la reserva aún no está confirmada
      const reservaPendiente = reservaTemporal.estado === EstadoReservaPsicologo.PENDIENTE_PAGO;
      
      this.logger.log(`Evaluación de pago - status obtenido de Flow: ${statusReal}, pagoExitoso: ${pagoExitoso}, reservaPendiente: ${reservaPendiente}, estado actual: ${reservaTemporal.estado}`);
      ...
      if (pagoExitoso) {
        if (reservaPendiente) {
          ...
          this.logger.log(`Pago exitoso detectado en retorno, confirmando reserva ${reservaTemporal.id}`);
          ...
          await this.pagoSesionService.confirmarPagoPorReservaId(reservaTemporal.id, datosPago);
            this.logger.log(`✅ Reserva ${reservaTemporal.id} confirmada desde endpoint return`);
            ...
            if (reservaTemporal) {
              ...
              if (reservaRecargada) {
                ...
                this.logger.log(`✅ Reserva recargada - Estado actualizado: ${reservaTemporal.estado}`);
              }
            }
          } catch (error) {
            // Si falla la confirmación, loguear pero continuar
            this.logger.error(`❌ Error al confirmar reserva desde return: ${error.message}`, error.stack);
            ...
            if (reservaTemporal) {
              ...
              if (reservaRecargada) {
                reservaTemporal = reservaRecargada;
                this.logger.log(`Reserva recargada después de error - Estado: ${reservaTemporal.estado}`);
              }
            }
          }
        } else {
          // Ya está confirmada, solo recargar para asegurar estado actualizado
          this.logger.log(`Reserva ${reservaTemporal.id} ya está confirmada, recargando estado...`);
          const reservaRecargada = await this.reservaRepository.findOne({
            where: { id: reservaTemporal.id },
            relations: ['psicologo', 'psicologo.usuario', 'paciente']
          });
        ...
        // Pago no exitoso
        this.logger.log(`⚠️ Pago no exitoso (status: ${status}), reserva ${reservaTemporal.id} permanece pendiente`);
      }
      ...
      if (!reservaTemporal) {
        ...
        const redirectUrl = `${frontUrl}/agenda/confirmacion-sesion?error=not-found`;
        this.logger.error('Reserva no encontrada después de procesar el retorno');
        return res.redirect(redirectUrl);
      }
      ...
      const redirectUrl = `${frontUrl}/agenda/confirmacion-sesion?reservaId=${reservaTemporal.id}`;
      this.logger.log(`Redirigiendo a frontend: ${redirectUrl} - Estado de reserva: ${reservaTemporal.estado}`);
      return res.redirect(redirectUrl);
