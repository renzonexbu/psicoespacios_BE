...
export class PacksService {
  constructor(
    ...
    private asignacionRepo: Repository<PackAsignacion>,
    @InjectRepository(PackAsignacionHorario)
    private horarioRepo: Repository<PackAsignacionHorario>,
    @InjectRepository(PackPagoMensual)
    private pagoMensualRepo: Repository<PackPagoMensual>,
  ...
  async asignarPack(dto: AsignarPackDto) {
    const pack = await this.packRepo.findOne({
      where: { id: dto.packId, activo: true },
    });
    ...
    if (!dto.horarios?.length)
      throw new BadRequestException('Debe indicar al menos un horario');
    ...
    if (boxes.length !== boxIds.length)
      throw new BadRequestException('Algún box indicado no existe');

    // Verificar conflictos antes de proceder
    const conflictos = await this.verificarConflictosReservas(
      dto.horarios,
      dto.fechaLimite,
    );
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      const horarios = dto.horarios.map((h) =>
        manager.create(PackAsignacionHorario, {
      ...
      const resultadoReservas = await this.generarReservasRecurrencia(
        manager,
        savedAsignacion.id,
        user.id,
        dto.fechaLimite,
      ...
      await this.generarPagosMensuales(
        manager,
        savedAsignacion.id,
        user.id,
        pack.precio,
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      await manager.update(
        Reserva,
        {
          ...
          fecha: Between(hoy, new Date(2999, 11, 31)),
        },
  ...
  async cancelarPack(dto: CancelarPackDto) {
    return this.dataSource.transaction(async (manager) => {
      ...
      if (!asignacion)
        throw new NotFoundException('Asignación de pack no encontrada');
  ...
  async getPacksByUsuario(usuarioId: string) {
    ...
    return {
      ...
      resumen: {
        ...
        packsActivos: packsDelUsuario.filter(
          (p) => p.estado === EstadoPackAsignacion.ACTIVA,
        ).length,
        packsCancelados: packsDelUsuario.filter(
          (p) => p.estado === EstadoPackAsignacion.CANCELADA,
        ).length,
        totalHorarios: packsDelUsuario.reduce(
          (sum, p) => sum + p.estadisticas.totalHorarios,
          0,
        ),
      },
    };
  ...
  private async verificarConflictosReservas(
    horarios: any[],
    fechaLimite?: string,
  ): Promise<any[]> {
        ...
        for (const reservaExistente of reservasExistentes) {
          if (
            ...
            conflictos.push({
              ...
              boxId: horario.boxId,
              boxNombre:
                reservaExistente.box?.nombre || `Box ${reservaExistente.boxId}`,
  ...
  private generarMensajeConflictos(conflictos: any[]): string {
    ...
    conflictos.forEach((conflicto) => {
      ...
      conflictosAgrupados[clave].horariosSolicitados.add(
        conflicto.horarioSolicitado,
      );
      conflictosAgrupados[clave].horariosExistentes.add(
        conflicto.horarioExistente,
      );
      conflictosAgrupados[clave].psicologos.add(
        conflicto.psicologoExistente.nombre,
      );
      conflictosAgrupados[clave].fechas.add(conflicto.fecha);
    });

    // Generar mensaje agrupado
    Object.values(conflictosAgrupados).forEach(
      (grupo: GrupoConflictos, index: number) => {
        mensaje += `${index + 1}. ${grupo.diaSemana} en Box "${grupo.boxNombre}":\n`;

        // Mostrar horarios solicitados
        const horariosSolicitados = Array.from(
          grupo.horariosSolicitados,
        ).sort();
    ...
    mensaje +=
      'Por favor, ajuste los horarios solicitados para evitar estos conflictos.';

    return mensaje;
  }

  private async generarReservasRecurrencia(
    manager: any,
    asignacionId: string,
    usuarioId: string,
    fechaLimite?: string,
    ...
    if (fechaLimite) {
      ...
      if (end > maxDate) {
        throw new BadRequestException(
          'La fecha límite no puede ser mayor a 1 año desde hoy',
        );
      }
    } else {
  ...
  private async generarPagosMensuales(
    manager: any,
    asignacionId: string,
    usuarioId: string,
    precioPack: number,
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      return {
        ...
        reservasAfectadas: {
          ...
          estado:
            'Todas las reservas del pack de este mes ahora se consideran pagadas',
        },
      };
    });
  }

  async reembolsarPagoMensual(
    pagoId: string,
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      if (pago.estado !== EstadoPagoPackMensual.PAGADO) {
        throw new BadRequestException(
          'Solo se pueden reembolsar pagos que estén marcados como pagados',
        );
      }
  ...
  async getPagosMensualesPorAsignacion(
    asignacionId: string,
  ): Promise<PackPagoMensual[]> {
  ...
  async getPagosMensualesPorUsuario(
    usuarioId: string,
    mes?: string,
  ): Promise<PackPagoMensual[]> {
