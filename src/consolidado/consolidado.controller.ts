...
import {
  QueryConsolidadoMensualDto,
  ConsolidadoMensualDto,
  QueryConsolidadoMensualGlobalDto,
...
export class ConsolidadoController {
  ...
  async getConsolidadoMensual(
    ...
    const psicologoId =
      req.user.role === 'PSICOLOGO' ? req.user.id : query.psicologoId;

    if (!psicologoId) {
      throw new BadRequestException('Psicólogo ID es requerido');
    }

    return this.consolidadoService.getConsolidadoMensual(
      psicologoId,
      query.mes,
    );
  }
  ...
  async getConsolidadoMensualGlobal(
    ...
    const data = await this.consolidadoService.getConsolidadoMensualGlobal(
      query.mes,
    );
    ...
    if (formato === 'csv') {
      ...
      res.setHeader(
        'Content-Disposition',
        `attachment; filename=consolidado_global_${query.mes}.csv`,
      );
