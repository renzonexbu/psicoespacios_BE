...
export class SeedAdditionalData1685394300000 implements MigrationInterface {
   ...
   */
  private async seedPerfilesDerivacion(
    ...
    if (psicologos.length === 0) {
      console.log(
        'No se encontraron psicólogos para crear perfiles. Omitiendo inserción.',
      );
      return;
    }
  ...
  private async seedSuscripciones(queryRunner: QueryRunner): Promise<void> {
    ...
    if (psicologos.length === 0 || planes.length === 0) {
      console.log(
        'No se encontraron psicólogos o planes para crear suscripciones. Omitiendo inserción.',
      );
      return;
    }
  ...
  private async seedPacientes(queryRunner: QueryRunner): Promise<void> {
    ...
    if (psicologos.length === 0) {
      console.log(
        'No se encontraron psicólogos para crear pacientes. Omitiendo inserción.',
      );
      return;
    }
    ...
    const usuarios = await queryRunner.query(
      "SELECT id FROM users WHERE role = 'PACIENTE'",
    );
    if (usuarios.length === 0) {
      console.log(
        'No se encontraron usuarios PACIENTE para crear pacientes. Omitiendo inserción.',
      );
      return;
    }
    const pacientes = [
  ...
  private async seedReservas(queryRunner: QueryRunner): Promise<void> {
    ...
    if (
      ...
      console.log(
        'No se encontraron psicólogos, boxes o suscripciones para crear reservas. Omitiendo inserción.',
      );
      return;
    }
    ...
    const reservas = [
      // Reserva pasada completada
      {
        ...
        fechaFin: formatDate(
          new Date(addDays(hoy, -10).getTime() + 60 * 60 * 1000),
        ), // +1 hora
        estado: 'COMPLETADA',
        notas: 'Sesión regular',
        psicologoId: psicologos[0].id,
        boxId: boxes[0].id,
        suscripcionId: suscripciones.find(
          (s) => s.usuarioId === psicologos[0].id,
        )?.id,
      },
      // Reserva pasada cancelada
      {
        ...
        fechaFin: formatDate(
          new Date(addDays(hoy, -5).getTime() + 60 * 60 * 1000),
        ), // +1 hora
        estado: 'CANCELADA',
        ...
        boxId: boxes[1].id,
        suscripcionId: suscripciones.find(
          (s) => s.usuarioId === psicologos[1].id,
        )?.id,
      },
      // Reserva para hoy
      {
        fechaInicio: formatDate(
          new Date(hoy.getFullYear(), hoy.getMonth(), hoy.getDate(), 15, 0, 0),
        ),
        fechaFin: formatDate(
          new Date(hoy.getFullYear(), hoy.getMonth(), hoy.getDate(), 16, 0, 0),
        ),
        estado: 'CONFIRMADA',
      ...
      // Reserva futura
      {
        ...
        fechaFin: formatDate(
          new Date(addDays(hoy, 3).getTime() + 60 * 60 * 1000),
        ), // +1 hora
        estado: 'PENDIENTE',
  ...
  private async seedPagos(queryRunner: QueryRunner): Promise<void> {
    ...
    const existingPagos = await queryRunner.query('SELECT COUNT(*) FROM pagos');
    ...
    if (suscripciones.length === 0) {
      console.log(
        'No se encontraron suscripciones para crear pagos. Omitiendo inserción.',
      );
      return;
    }
    ...
    for (const suscripcion of suscripciones) {
      const fechaCompletado =
        suscripcion.id % 2 === 0 ? `'${new Date().toISOString()}'` : 'NULL';
        ...
        datosTarjeta:
          suscripcion.id % 2 === 0
            ? {
                ...
                marca:
                  parseInt(suscripcion.id.substring(0, 2), 16) % 2 === 0
                    ? 'VISA'
                    : 'MASTERCARD',
              }
            : null,
