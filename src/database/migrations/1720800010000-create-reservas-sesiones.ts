...
export class CreateReservasSesiones1720800010000 implements MigrationInterface {
  name = 'CreateReservasSesiones1720800010000';

  public async up(queryRunner: QueryRunner): Promise<void> {
    // Crear enum para EstadoReservaPsicologo (si no existe)
    ...
    // Crear enum para ModalidadSesion (si no existe)
    await queryRunner.query(`
      DO $$ BEGIN
    ...
    // Crear tabla reservas_sesiones (si no existe)
    await queryRunner.query(`
    ...
    await queryRunner.query(`
      CREATE INDEX IF NOT EXISTS "IDX_reservas_sesiones_psicologo_id" ON "reservas_sesiones" ("psicologo_id")
    `);

    await queryRunner.query(`
      CREATE INDEX IF NOT EXISTS "IDX_reservas_sesiones_paciente_id" ON "reservas_sesiones" ("paciente_id")
    `);

    await queryRunner.query(`
      CREATE INDEX IF NOT EXISTS "IDX_reservas_sesiones_fecha" ON "reservas_sesiones" ("fecha")
    `);

    await queryRunner.query(`
      CREATE INDEX IF NOT EXISTS "IDX_reservas_sesiones_box_id" ON "reservas_sesiones" ("box_id")
    `);

    await queryRunner.query(`
      CREATE INDEX IF NOT EXISTS "IDX_reservas_sesiones_estado" ON "reservas_sesiones" ("estado")
    `);

    await queryRunner.query(`
      CREATE INDEX IF NOT EXISTS "IDX_reservas_sesiones_modalidad" ON "reservas_sesiones" ("modalidad")
    `);

    // Índice compuesto para consultas de disponibilidad
    await queryRunner.query(`
      CREATE INDEX IF NOT EXISTS "IDX_reservas_sesiones_disponibilidad" 
      ON "reservas_sesiones" ("psicologo_id", "fecha", "hora_inicio", "hora_fin")
    `);

    // Índice compuesto para consultas de disponibilidad de box
    await queryRunner.query(`
      CREATE INDEX IF NOT EXISTS "IDX_reservas_sesiones_box_disponibilidad" 
