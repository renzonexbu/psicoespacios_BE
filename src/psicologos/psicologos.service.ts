...
export class PsicologosService {
  ...
  create(createPsicologoDto: CreatePsicologoDto) {
    ...
    const existePsicologo = this.psicologos.some(
      (p) =>
        p.nombre &&
        p.nombre.toLowerCase() === createPsicologoDto.nombre?.toLowerCase(),
    );
    ...
    try {
      ...
      if (Math.random() < 0.05) {
        ...
        throw new DatabaseConnectionException(
          'Error al conectar con la base de datos para crear psicólogo',
        );
      }
      ...
      if (!(error instanceof DatabaseConnectionException)) {
        ...
        throw new BadRequestException(
          'No se pudo crear el psicólogo debido a datos inválidos',
        );
      }
      throw error;
    }
  ...
  consultarDiasDisponibles(
    query: QueryDisponibilidadDiasDto,
  ): DisponibilidadDia[] {
    ...
    try {
      ...
      if (!['Online', 'Presencial'].includes(query.modalidad)) {
        throw new BadRequestException(
          'La modalidad debe ser "Online" o "Presencial"',
        );
      }
      ...
      throw new DatabaseConnectionException(
        'Error al conectar con la base de datos para consultar disponibilidad',
      );
    }
  }

  consultarHorariosDisponibles(
    query: QueryDisponibilidadHorariosDto,
  ): HorarioSlot[] {
    ...
    try {
      ...
      throw new DatabaseConnectionException(
        'Error al conectar con la base de datos para consultar horarios disponibles',
      );
    }
  }
  ...
  reservarTurno(createReservaDto: CreateReservaDto) {
    ...
    try {
      ...
      // Validar que el psicólogo existe
      const existePsicologo = this.psicologos.some(
        (p) => p.id === createReservaDto.psicologoId,
      );
      if (!existePsicologo) {
      ...
      throw new DatabaseConnectionException(
        'Error al conectar con la base de datos para reservar turno',
      );
    }
  }
  ...
  historialTurnosPaciente(pacienteId: string, query: QueryHistorialTurnosDto) {
    ...
    try {
      ...
      const reservasPaciente = this.reservas.filter(
        (r) => r.pacienteId === pacienteId,
      );
      ...
      throw new DatabaseConnectionException(
        'Error al conectar con la base de datos para consultar historial de turnos',
      );
    }
  }
  ...
  findAll(): Psicologo[] {
    try {
      ...
      throw new DatabaseConnectionException(
        'Error al conectar con la base de datos para listar psicólogos',
      );
    }
  }
  ...
  findOne(id: string): Psicologo {
    try {
      ...
      throw new DatabaseConnectionException(
        'Error al conectar con la base de datos para buscar psicólogo',
      );
    }
  }
  ...
  update(id: string, updatePsicologoDto: UpdatePsicologoDto): Psicologo {
    try {
      ...
      if (updatePsicologoDto.nombre) {
        const existeNombre = this.psicologos.some(
          (p, idx) =>
            idx !== psicologoIndex &&
            p.nombre &&
            p.nombre.toLowerCase() === updatePsicologoDto.nombre?.toLowerCase(),
        );

        if (existeNombre) {
          throw new ConflictException(
            'Ya existe otro psicólogo con ese nombre',
          );
        }
      }
      ...
      throw new DatabaseConnectionException(
        'Error al conectar con la base de datos para actualizar psicólogo',
      );
    }
  }
  ...
  remove(id: string): { message: string } {
    try {
      ...
      const tieneReservasActivas = this.reservas.some(
        (r) =>
          r.psicologoId === id && [EstadoReserva.CONFIRMADA].includes(r.estado),
      );

      if (tieneReservasActivas) {
        throw new ConflictException(
          'No se puede eliminar el psicólogo porque tiene reservas activas',
        );
      }
      ...
      throw new DatabaseConnectionException(
        'Error al conectar con la base de datos para eliminar psicólogo',
      );
    }
  }
}
