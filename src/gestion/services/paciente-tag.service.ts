...
export class PacienteTagService {
  ...
  async asignarTag(
    ...
    this.logger.log(
      `Asignando tag '${updateTagDto.tag}' al paciente ${pacienteId} por psicólogo ${psicologoUserId}`,
    );
    ...
    if (paciente.idUsuarioPsicologo !== psicologo.id) {
      throw new ForbiddenException(
        'No tienes permisos para asignar tags a este paciente',
      );
    }
    ...
    this.logger.log(
      `Tag '${updateTagDto.tag}' asignado exitosamente al paciente ${pacienteId}`,
    );
  ...
  async removerTag(
    ...
    this.logger.log(
      `Removiendo tag del paciente ${pacienteId} por psicólogo ${psicologoUserId}`,
    );
    ...
    if (paciente.idUsuarioPsicologo !== psicologo.id) {
      throw new ForbiddenException(
        'No tienes permisos para remover tags de este paciente',
      );
    }
    ...
    this.logger.log(
      `Tag '${tagAnterior}' removido exitosamente del paciente ${pacienteId}`,
    );
  ...
  async obtenerTag(
    ...
    this.logger.log(
      `Obteniendo tag del paciente ${pacienteId} por psicólogo ${psicologoUserId}`,
    );
    ...
    if (paciente.idUsuarioPsicologo !== psicologo.id) {
      throw new ForbiddenException(
        'No tienes permisos para ver el tag de este paciente',
      );
    }
    ...
    if (!usuarioPaciente) {
      throw new NotFoundException(
        'Información del usuario paciente no encontrada',
      );
    }
  ...
  async listarPacientesConTags(
    ...
    Array<{ id: string; nombre: string; apellido: string; tag: string | null }>
  > {
    this.logger.log(
      `Listando pacientes con tags para psicólogo ${psicologoUserId}`,
    );

    const pacientes = await this.pacienteRepository
      .createQueryBuilder('paciente')
      .leftJoinAndSelect('paciente.idUsuarioPaciente', 'usuario')
      .where('paciente.idUsuarioPsicologo = :psicologoUserId', {
        psicologoUserId,
      })
