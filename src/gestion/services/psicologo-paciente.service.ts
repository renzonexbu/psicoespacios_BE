...
export class PsicologoPacienteService {
  ...
  async getPsicologoByPacienteUsuario(usuarioId: string) {
    this.logger.log(
      `Buscando psicólogo para paciente con usuarioId: ${usuarioId}`,
    );
    ...
    try {
      ...
      if (!paciente) {
        throw new NotFoundException(
          `Paciente no encontrado para el usuario: ${usuarioId}`,
        );
      }

      this.logger.log(
        `Paciente encontrado: ${paciente.id}, idUsuarioPsicologo: ${paciente.idUsuarioPsicologo}`,
      );
      ...
      this.logger.log(
        `Buscando psicólogo con ID = ${paciente.idUsuarioPsicologo}`,
      );
      ...
      if (psicologo) {
        this.logger.log(
          `ID del psicólogo: ${psicologo.id}, Usuario: ${psicologo.usuario.nombre}, UsuarioID: ${psicologo.usuario.id}`,
        );
      }

      if (!psicologo) {
        this.logger.warn(
          `Psicólogo no encontrado para ID: ${paciente.idUsuarioPsicologo}`,
        );
        return {
          paciente: {
            id: paciente.id,
      ...
      const countSesiones = await this.reservaPsicologoRepository
        ...
        .where('reserva.psicologo = :psicologoId', {
        ...
        .andWhere('reserva.estado IN (:...estados)', {
          estados: [
            ...
            EstadoReservaPsicologo.COMPLETADA,
          ],
        })
        .getCount();

      this.logger.log(
        `Sesiones del psicólogo ${psicologo.id} hasta hoy: ${countSesiones}`,
      );
      ...
      this.logger.log(
        `Buscando disponibilidad con usuario.id = ${psicologo.usuario.id}`,
      );

      const disponibilidad = await this.disponibilidadRepository
        .createQueryBuilder('disp')
        .where('disp.psicologo_id = :usuarioId', {
      ...
      this.logger.log(
        `Disponibilidad encontrada: ${disponibilidad.length} días`,
      );
      ...
      this.logger.error(
        `Error al buscar psicólogo para paciente ${usuarioId}: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  async getPsicologoByPacienteId(pacienteId: string) {
    ...
    try {
      ...
      this.logger.error(
        `Error al buscar psicólogo para paciente ${pacienteId}: ${error.message}`,
      );
      throw error;
    }
  }
