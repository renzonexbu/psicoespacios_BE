...
export class BoxesService {
  ...
  /**
   * Obtener un box por ID incluyendo los eliminados (para sesiones)
   */
  async findOneWithDeleted(id: string): Promise<Box> {
    const box = await this.boxesRepository.findOne({
      where: { id }, // Incluir todos los boxes, incluso eliminados
      relations: ['sede'],
  ...
  /**
   * Obtener todos los boxes incluyendo los eliminados (para administradores)
   */
  ...
  /**
   * Restaurar un box eliminado (soft delete)
   */
  async restore(id: string): Promise<Box> {
    ...
    if (!box.deletedAt) {
      throw new BadRequestException('El box no está eliminado');
    }
