...
function initializeDatabase() {
  try {
    ...
    // Esperar un poco más para asegurar que PostgreSQL esté completamente iniciado
    console.log(
      'Esperando unos segundos adicionales para asegurar que PostgreSQL esté listo...',
    );
    spawnSync('timeout', ['5'], { shell: true });

    // En Windows/PowerShell la forma de pasar comandos a Docker es diferente
    // Usar un script bash dentro del contenedor
    const command = `
      docker exec psicoespacios_db bash -c "
        until pg_isready -U psicoespacios_user -h localhost; do
          echo 'Esperando a que PostgreSQL esté disponible...'
          sleep 2
        done
        psql -U psicoespacios_user -d psicoespacios -c 'CREATE EXTENSION IF NOT EXISTS \\\"uuid-ossp\\\";'
      "
    `;
